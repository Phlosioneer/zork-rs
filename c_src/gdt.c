...
/* 						!COL HDRS. */
	...
	supp_printf("%3d", i);
	for (l = 1; l <= 6; ++l)
	    supp_printf(" %6d", eqr[i + l * 200 - 201]);
	supp_printf("\n");
...
/* 						!COL HDRS */
    ...
    for (i = j; i <= i__1; ++i) {
	more_output(NULL);
	supp_printf("%3d%6d%6d%6d%4d%7d%7d%4d%4d%6d%6d %4d%4d%4d%6d\n",
...
/* 						!COMPUTE END OF LINE. */
	more_output(NULL);
	supp_printf("%3d-%3d  ", i, l);
	for (l1 = i; l1 <= l; ++l1)
	    supp_printf("%7d", exits_1.travel[l1 - 1]);
	supp_printf("\n");
/* L14100: */
    }
    goto L2000;
...
L15000:
    more_output(NULL);
    supp_printf("THFPOS= %d, THFFLG= %c, THFACT= %c\n",
	   hack_1.thfpos, hack_1.thfflg ? 'T' : 'F',
	   hack_1.thfact ? 'T' : 'F');
    more_output(NULL);
    supp_printf("SWDACT= %c, SWDSTA= %d\n", hack_1.swdact ? 'T' : 'F',
	   hack_1.swdsta);
    goto L2000;
...
L19000:
    more_output(NULL);
    supp_printf("Parse vector= %6d %6d %6d      %c %6d\n",
	   prsvec_1.prsa, prsvec_1.prso, prsvec_1.prsi,
	   prsvec_1.prswon ? 'T' : 'F', prsvec_1.prscon);
    more_output(NULL);
    supp_printf("Play vector=  %6d %6d      %c\n", play_1.winner, play_1.here,
	    play_1.telflg ? 'T' : 'F');
    more_output(NULL);
    supp_printf("State vector= %6d %6d %6d %6d %6d %6d %6d %6d %6d\n",
    ...
    more_output(NULL);
    supp_printf("Scol vector=  %6d %6d %6d\n", screen_1.fromdr,
...
/* 						!ENTRY NO VALID? */
    supp_printf("Old= %c      New= ", flags[j - 1] ? 'T' : 'F');
/* 						!TYPE OLD, GET NEW. */
    (void) fflush(stdout);
    (void) fgets(buf, sizeof buf, stdin);
    more_input();
...
/* 						!INDICES VALID? */
    supp_printf("Old = %6d      New = ", eqr[j + k * 200 - 201]);
/* 						!TYPE OLD, GET NEW. */
    (void) fflush(stdout);
    (void) fgets(buf, sizeof buf, stdin);
    more_input();
...
/* 						!INDICES VALID? */
    supp_printf("Old = %6d      New = ", eqo[j + k * 200 - 201]);
    (void) fflush(stdout);
    (void) fgets(buf, sizeof buf, stdin);
    more_input();
...
/* 						!INDICES VALID? */
    supp_printf("Old = %6d      New = ", eqa[j + (k << 2) - 5]);
    (void) fflush(stdout);
    (void) fgets(buf, sizeof buf, stdin);
    more_input();
...
/* 						!FLAGS ENTRY? */
    supp_printf("Old = %6d      New = ", eqc[j + k * 25 - 26]);
    (void) fflush(stdout);
    (void) fgets(buf, sizeof buf, stdin);
    more_input();
...
/* 						!ENTRY NO VALID? */
    supp_printf("Old= %6d     New= ", exits_1.travel[j - 1]);
    (void) fflush(stdout);
    (void) fgets(buf, sizeof buf, stdin);
    more_input();
...
/* 						!INDICES VALID? */
    supp_printf("Old = %6d      New= ", eqv[j + (k << 2) - 5]);
    (void) fflush(stdout);
    (void) fgets(buf, sizeof buf, stdin);
    more_input();
...
/* 						!VALID ENTRY? */
    supp_printf("Old= %6d      New= ", switch_[j - 1]);
    (void) fflush(stdout);
    (void) fgets(buf, sizeof buf, stdin);
    more_input();
...
/* Computing MIN */
	...
	supp_printf("%3d-%3d  ", i, l);
	for (l1 = i; l1 <= l; ++l1)
	    supp_printf(" %6d", rmsg_1.rtext[l1 - 1]);
	supp_printf("\n");
/* L41100: */
    }
    goto L2000;
...
L43000:
    supp_printf("Old= %6d      New= ", play_1.here);
    (void) fflush(stdout);
    (void) fgets(buf, sizeof buf, stdin);
    more_input();
...
L44000:
    more_output(NULL);
    supp_printf("ORPHS= %7d%7d%7d%7d%7d%7d\n",
	   orp[0], orp[1], orp[2], orp[3], orp[4], last_1.lastit);
    more_output(NULL);
    supp_printf("PV=    %7d%7d%7d%7d%7d\n",
	   pvec[0], pvec[1], pvec[2], pvec[3], pvec[4]);
    more_output(NULL);
    supp_printf("SYN=   %7d%7d%7d%7d%7d%7d\n",
	   syn[0], syn[1], syn[2], syn[3], syn[4], syn[5]);
    more_output(NULL);
    supp_printf("              %7d%7d%7d%7d%7d\n",
...
L45000:
    supp_printf("Old= %6d      New= ", debug_1.prsflg);
/* 						!TYPE OLD, GET NEW. */
    (void) fflush(stdout);
    (void) fgets(buf, sizeof buf, stdin);
    more_input();
...
/* 						!DISPLAY PUZZLE */
	...
	supp_printf(" ");
	for (j = i; j <= i + 7; ++j)
	     supp_printf("%3d", puzzle_1.cpvec[j - 1]);
	supp_printf("\n");
/* L46100: */
    }
    goto L2000;
...
/* 						!VALID ENTRY? */
    supp_printf("Old= %6d      New= ", puzzle_1.cpvec[j - 1]);
/* 						!OUTPUT OLD, */
    (void) fflush(stdout);
    (void) fgets(buf, sizeof buf, stdin);
    more_input();
